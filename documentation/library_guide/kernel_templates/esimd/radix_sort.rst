...
for intermediate data storage. For the algorithms to operate correctly, there must be enough memory on the device.
They throw a ``std::bad_alloc`` exception if there is not enough global device memory.
